...
class AccIntegration:
    ...
    def predict(self, imu_data: np.ndarray) -> None:
        ...
        velocity = scipy.integrate.trapezoid(imu_data).reshape(3,1)
        self.velocity = np.append(self.velocity, velocity, axis = 1)
        self.velocity = scipy.signal.detrend(self.velocity)
        position = scipy.integrate.trapezoid(self.velocity[:, -self.time_limit:]).reshape(3,1)
...
class GyroIntegration:
    ...
    def predict(self, gyro_data: np.ndarray) -> None:
